...
class OutputManagerError(OutputError):
...
class OutputManager:
    ...
    async def save_wav(
        ...
        """
        if not audio_data:
            raise OutputManagerError("No audio data provided", {"original_name": original_name})
    ...
    async def save_sidecar(
        ...
        sidecar_path = output_path.with_stem(f"{stem}.json")
    ...
    async def move_to_processed(self, file_path: Path) -> None:
        ...
        if not self.processed_dir:
            raise OutputManagerError(
                "Processed directory not configured",
